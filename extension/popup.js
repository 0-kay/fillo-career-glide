...
class FilloPopup {
  ...
  updateProfilePreview(profile) {
    ...
    const filledFields = [details.first_name, details.last_name, details.email, details.phone, details.address].filter(Boolean).length;
      ...
      if (response?.success) {
        this.showStatus(
            `✅ Filled ${response.filled || 0}/${response.attempted || 0} fields${
                typeof response.aiMatches === "number" ? ` (${response.aiMatches} via AI)` : ""
            }`,
            "success"
        );
      } else {
      ...
      const results = await chrome.scripting.executeScript({
        ...
        func: () => {
          try {
            ...
            return { totalFields: formFields.length, url: window.location.href };
          } catch (e) {
            return { totalFields: 0, error: e?.message };
          }
        },
      });
      const total = results.reduce((sum, r) => sum + (r.result?.totalFields || 0), 0);
      this.showStatus(`Found ${total} form fields across ${results.length} frame(s)`, "success");
    } catch (err) {
